...
public class Max extends BaseBuiltin {
    ...
    public boolean bodyCall(Node[] args, int length, RuleContext context) {
    	...
    	if (maxVal instanceof Float) {
    		min = Utils.makeFloatNode(maxVal.floatValue());
    	}
    	else if ( maxVal instanceof Double) {
    		min = Util.makeDoubleNode(maxVal.doubleValue());
    	}
    	else {
    		min = Util.makeLongNode(maxVal.longValue());
    	}
        return env.bind(args[length - 1], min);
    }

    private Number maxOfList(Node lst, RuleContext context) {

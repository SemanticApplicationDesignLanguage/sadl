...
public class ConfigurationManagerForEditing extends ConfigurationManager implements IConfigurationManagerForEditing {
	...
	public ConfigurationManagerForEditing(String modelFolderPathname, String repoType) throws ConfigurationException {
		super(modelFolderPathname, repoType);
		init(modelFolderPathname);
	}
	
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#setTranslatorClassName(java.lang.String)
	 */
	public boolean setTranslatorClassName(String translatorClassName) throws ConfigurationException {
        ...
        if (sitr.hasNext()) {
            ...
            getConfigModel().add(reasonerCategory, translatorClassNameProperty, tcn);
        	bChanged = true;
        }
        if (bChanged) {	
	...
	protected boolean saveConfigurationModel() {
        ...
        try {
    		String configFilename = getModelFolderPath().getCanonicalPath() + File.separator + CONFIG_FILENAME;
            // save model
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#setReasonerClassName(java.lang.String)
	 */
	public boolean setReasonerClassName(String reasonerClassName) {
		boolean bChanged = false;
        Property reasonerClassNameProperty = getConfigModel().createProperty(pREASONER_CLASSNAME);
        Literal rcn = getConfigModel().createTypedLiteral(reasonerClassName);
        
        Resource reasonerSpec = getReasonerSpecResource();

        List<Statement> stmts = new ArrayList<Statement>();
        StmtIterator sitr = getConfigModel().listStatements(reasonerSpec, reasonerClassNameProperty, (RDFNode)null);
        int stmtcntr = 0;
        if (sitr.hasNext()) {
        	Statement stmtToChange = null;
	        while (sitr.hasNext()) {
	        ...
	        // remove triple for each stmt after the first--only one reasoner is allowed
	        for (int i = 0; stmts != null && i < stmts.size(); i++) {
	        	Statement stmt = stmts.get(i);
	            getConfigModel().remove(stmt);
	            bChanged = true;
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#isConfigurationStale()
	 */
	...
	protected String getConfigurationFilename() throws IOException {
		return getModelFolderPath().getCanonicalPath() + File.separator + CONFIG_FILENAME;
	}
	
	protected boolean init(String modelFolderPathname) throws ConfigurationException {
		...
		try {
			...
			File configFile = new File(configFilename);
			if (configFile.exists()) {
                timeConfigFileLastModifiedAtInitialization = configFile.lastModified();
			}
		} catch (IOException e) {
			e.printStackTrace();
			throw new ConfigurationException("Failed to get configuration File in folder '" + modelFolderPathname + "'", e);
		} catch (Throwable t) {
			t.printStackTrace();
			throw new ConfigurationException("Unexpected configuration File exception in folder '" + modelFolderPathname + "': " + t.getMessage());
		}
		return true;
		...
		if (isConfigurationStale()) {
			...
			try {
				...
				if (configFile.exists()) {
					...
					throw new ConfigurationException("Configuration file '" + configFilename + "' not found.");
				}
			} catch (IOException e) {
    ...
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#resetJena()
	 */
    ...
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#clearJenaModelContent(com.hp.hpl.jena.ontology.OntModel)
	 */
    public boolean clearJenaModelContent(OntModel model) {
		...
		while (smitr.hasNext()) {
     	...
     	try {
         	model.removeAll();	// also clear all statements from the model so it's as if it were freshly created.
     	}
     	catch (Throwable t) {
     		t.printStackTrace();
     	}
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#addMapping(com.hp.hpl.jena.rdf.model.Resource, com.hp.hpl.jena.rdf.model.Resource, com.hp.hpl.jena.rdf.model.Literal)
	 */
    ...
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#addJenaMapping(java.lang.String, java.lang.String)
	 */
    public boolean addJenaMapping(String publicUri, String altUrl) throws IOException, URISyntaxException, ConfigurationException {
    	if (publicUri == null) {
    		throw new ConfigurationException("Mapping can't have a null publicURI.");
    	}
    	if (altUrl == null) {
    		throw new ConfigurationException("Mapping of publicURI '" + publicUri + "' can't be to null.");
    	}
    	OntDocumentManager.getInstance().addAltEntry(publicUri, altUrl);
        return true;
    }
    
    protected boolean deleteMapping(String publicUri, String altUrl) throws IOException, URISyntaxException, ConfigurationException {
    	if (publicUri == null) {
    		throw new ConfigurationException("Mapping can't have a null publicURI.");
    	}
    	Resource altv = getMappingModel().createResource(altUrl);
    	StmtIterator sitr = getMappingModel().listStatements(null, altUrlProp, altv);
    	if (sitr.hasNext()) {
    	...
    	OntDocumentManager.getInstance().getFileManager().removeCacheModel(publicUri);
        return true;
    }
    
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#deleteModel(java.lang.String)
	 */
    public boolean deleteModel(String publicUri) throws ConfigurationException, IOException, URISyntaxException {
    	String altUrl = getAltUrlFromPublicUri(publicUri);
    	if (repoType.equals(IConfigurationManager.JENA_TDB)) {
    		try {
	    		OntModel modelToDelete = getModelGetter().getOntModel(publicUri, altUrl, IConfigurationManager.JENA_TDB);
	...
	/**
	 * Method to get a list of all the available reasoner plugins using a {@link ServiceLoader}
	...
	public static List<AvailablePlugin> getAvailableReasonerPlugins() {
		...
		for (Iterator<IReasoner> itr = reasoners.iterator(); itr.hasNext(); ) {
			...
			AvailablePlugin avplg = new AvailablePlugin(PluginType.Reasoner, category, family, clsNm);
			rval.add(avplg);
		}
		return rval;
	}
	...
	/**
	 * Method to get a list of all the available translator plugins using a {@link ServiceLoader}
	 * 
	 * @return A list of all available translator plugins that belong to the same family as the current reasoner
	 */
	public static List<AvailablePlugin> getAvailableTranslatorPlugins() {
		...
		for( Iterator<ITranslator> itr = translators.iterator(); itr.hasNext() ; ){
			...
			AvailablePlugin avplg = new AvailablePlugin(PluginType.Translator, category, family, clsNm);
			rval.add(avplg);
		}	
		return rval;
	}
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#getAvailableTranslatorPluginsForCurrentReasoner()
	 */
	public List<AvailablePlugin> getAvailableTranslatorPluginsForCurrentReasoner() throws ConfigurationException {
		...
		for( Iterator<ITranslator> itr = translators.iterator(); itr.hasNext() ; ){
			...
			String category = trans.getConfigurationCategory();
			String family = trans.getReasonerFamily();
			if (family != null && family.equals(reasonerInst.getReasonerFamily())) {
				String clsNm = trans.getClass().getCanonicalName();
				AvailablePlugin avplg = new AvailablePlugin(PluginType.Translator, category, family, clsNm);
				rval.add(avplg);
			}
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#getAvailableBuiltinsForCurrentReasoner()
	 */
	public List<BuiltinInfo> getAvailableBuiltinsForCurrentReasoner() throws ConfigurationException {
		...
		if( serviceLoader != null ){
			for( Iterator<Class<?>> itr = serviceLoader.iterator(); itr.hasNext() ; ){
				try {
					Object trans = itr.next();
					BuiltinInfo binfo = reasonerInst.getBuiltinInfo(trans.getClass());
					if (binfo != null) {
						rval.add(binfo);
					}
				}
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#addConfiguration(com.ge.research.sadl.reasoner.ConfigurationItem)
	 */
	...
	public void addConfiguration(ConfigurationItem newItem) throws ConfigurationException {
		if (getConfigModel() != null) {
			...
			for (int i = 0; i < categoryHierarchy.length; i++) {
				...
				if (i < (categoryHierarchy.length - 1)) {
					// don't add the last one unless it's a SingleValue, in which case do it before
					newCategoryResource = getConfigModel().createResource(CONFIG_NAMESPACE + newCategory, type);
					if (i > 0) {
						lastCategoryResource = getConfigModel().createResource(CONFIG_NAMESPACE + lastCategory, type);
						getConfigModel().add(lastCategoryResource, subcatprop, newCategoryResource);
					}
				}
			...
			if (lastCategory != null) {
				...
				for (int i = 0; nvplist != null && i < nvplist.size(); i++) {
					...
					Property prop = getConfigModel().createProperty(CONFIG_NAMESPACE + nvp.getName());
					if (nvp.getValue() != null) {
						if (nvp.getConfigType().equals(ConfigurationType.Bag)) {
							if (beforeLastCategory != null) {
								Statement stmt = beforeLastCategory.getProperty(subcatprop);
								RDFNode bag;
								if (stmt != null) {
									bag = stmt.getObject();
								}
									...
									getConfigModel().add(beforeLastCategory, subcatprop, bag);
								}
								if (bag != null && bag instanceof Resource && ((Resource)bag).hasProperty(RDF.type, RDF.Bag)) {
									if (subject == null) {
										Resource bagItemType = getConfigModel().createResource(CONFIG_NAMESPACE + lastCategory);
										subject = getConfigModel().createResource(bagItemType);
										((Bag)bag.as(Bag.class)).add(subject);
									}
									getConfigModel().add(subject, prop, getConfigModel().createTypedLiteral(nvp.getValue()));
								}
							}
							else if (lastCategory != null) {
								subject = getConfigModel().createResource(CONFIG_NAMESPACE + lastCategory);
								...
								if (bag != null && bag instanceof Resource && ((Resource)bag).hasProperty(RDF.type, RDF.Bag)) {
									((Bag)bag.as(Bag.class)).add(getConfigModel().createTypedLiteral(nvp.getValue()));
								}
							}
							else {
								throw new ConfigurationException("Unable to add Bag to single-level category (" + lastCategory + ")");
							}
						}
						else if (nvp.getConfigType().equals(ConfigurationType.Sequence)) {
							if (beforeLastCategory != null) {
								Statement stmt = beforeLastCategory.getProperty(subcatprop);
								RDFNode seq;
								if (stmt != null) {
									...
									getConfigModel().add(beforeLastCategory, subcatprop, seq);
								}
								if (seq != null && seq instanceof Resource && ((Resource)seq).hasProperty(RDF.type, RDF.Seq)) {
									if (subject == null) {
										Resource bagItemType = getConfigModel().createResource(CONFIG_NAMESPACE + lastCategory);
										subject = getConfigModel().createResource(bagItemType);
								...
								if (seq != null && seq instanceof Resource && ((Resource)seq).hasProperty(RDF.type, RDF.Seq)) {
									((Seq)seq.as(Seq.class)).add(getConfigModel().createTypedLiteral(nvp.getValue()));
								}
								// check the size limit
								Statement sslStmt = subject.getProperty(getConfigModel().createProperty(CONFIG_NAMESPACE + pSeqSizeLimit));
								if (sslStmt != null) {
									RDFNode sizeLimitNode = sslStmt.getObject();
									if (sizeLimitNode instanceof Literal) {
										int sslval = ((Literal)sizeLimitNode).getInt();
										int cursize = ((Seq)seq.as(Seq.class)).size();
										if (cursize >= sslval) {
								...
								throw new ConfigurationException("Unable to add Sequence to single-level category (" + lastCategory + ")");
							}
						}
						else {
							if (subject == null) {
								if (beforeLastCategory == null && lastCategory != null) {
									subject = getConfigModel().createResource(CONFIG_NAMESPACE + lastCategory, type);
								}
								else {
									getConfigModel().add(beforeLastCategory, subcatprop, subject);
								}
							}
							subject.removeAll(prop);
							subject.addProperty(prop, getConfigModel().createTypedLiteral(nvp.getValue()));		
						}	
					}
					else {
						// the value is null
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#updateConfiguration(com.ge.research.sadl.reasoner.ConfigurationItem)
	 */
	public void updateConfiguration(ConfigurationItem newItem) throws ConfigurationException {
		if (getConfigModel() != null) {
			Resource type = getConfigModel().createResource(CATEGORY_KW);
			Property subcatprop =  getConfigModel().createProperty(SUBCATEGORY_PROP);  // we'll use this repeatedly
			String[] categoryHierarchy = newItem.getCategoryHierarchy();
			...
			List<NameValuePair> nvplist = newItem.getNameValuePairs();
			for (int i = 0; i < categoryHierarchy.length; i++) {
				Resource newCategory = getConfigModel().createResource(CONFIG_NAMESPACE + categoryHierarchy[i], type);
				if (i > 0) {
				...
				for (int i = 0; nvplist != null && i < nvplist.size(); i++) {
					...
					if (stmt != null) {
						if (nvp.getConfigType().equals(ConfigurationType.Bag)) {
							...
							Statement updateStmt = lastCategory.getProperty(getConfigModel().createProperty(CONFIG_NAMESPACE + nvp.getName()));
							updateStmt.changeObject(getConfigModel().createTypedLiteral(nvp.getValue()));
						}
					}
					else {
						getConfigModel().add(lastCategory, prop, getConfigModel().createTypedLiteral(nvp.getValue()));
					}
				}
			}
			clearReasoner();
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#saveConfiguration()
	 */
...
//                ResourceManager.refreshResource(configFilename);
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#saveOntPolicyFile()
	 */
	public boolean saveOntPolicyFile() {
		...
		try {
			mappingFilename = getModelFolderPath().getCanonicalPath() + File.separator +ONT_POLICY_RDF;
...
//                	ResourceManager.refreshResource(mappingFilename);
                	...
                	logger.error("Failed to refresh ont-policy file: " + t.getLocalizedMessage());
                }
            }
			setMappingChanged(false);
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#isConfigChanged()
	 */
	...
	protected void validateModelFolderPath(String modelFolderPathname) throws ConfigurationException {
		...
		if (!mfpnf.exists()) {
			...
			if (!stat) {
				throw new ConfigurationException("Unable to create model folder '" + modelFolderPathname + "'");
			}
		}
...
//		setModelFolderPath(mfpnf);
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#getProjectFolderPath()
	 */
	...
	public void addGlobalPrefix(String modelName, String globalPrefix) {
		...
		else if (globalPrefixes != null && globalPrefixes.containsKey(modelName)) {
		...
		if (bChanged) {
			...
			setMappingChanged(true);
        	logger.debug("Changed global prefix for '" + modelName + "' to '" + globalPrefix + "'");
		}
	...
	private void setMappingPrefix(String modelName, String globalPrefix) {
		...
		StmtIterator sitr = getMappingModel().listStatements(null, publicUrlProp, publicUri);
		...
		if (ontSpec != null) {
			ontSpec.addProperty(prefixProp, getMappingModel().createTypedLiteral(globalPrefix));
		}
	}
	...
	/* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#setProjectFolderPath(java.lang.String)
	 */
    ...
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#getNamedConceptsInModel(com.hp.hpl.jena.ontology.OntModel, java.lang.String, com.ge.research.sadl.utils.SadlUtils.ConceptType, com.ge.research.sadl.reasoner.ConfigurationManagerForEditing.Scope)
	 */
	public List<ConceptName> getNamedConceptsInModel(OntModel localModel, String modelName, ConceptType cType, Scope scope) throws InvalidNameException {
    	List<ConceptName> lst = new ArrayList<ConceptName>();
    	List<ConceptType> typesToInclude;
    	logger.debug("getNamedConceptsInModel called with ConceptType '" + cType + "'");
    	
    	// this is for concepts in models, not models (default model name or possible imports)
    	if (cType == null) {
    		// null type -> all types
    	...
    	String modelns = ConfigurationManager.addHashToNonTerminatedNamespace(modelName);
    	
    	for (int i = 0; i < typesToInclude.size(); i++) {
    		ConceptType actype = typesToInclude.get(i);
	     	switch (actype) {
	    	case ANNOTATIONPROPERTY:
	    		ExtendedIterator<AnnotationProperty> apitr = localModel.listAnnotationProperties();
	    		if (apitr.hasNext()) {
	    			while (apitr.hasNext()) {
	    				AnnotationProperty aprop = apitr.next();
	    				if (!aprop.isAnon()) {
	    					ConceptName inclname = getNamedConceptFromModel(localModel, aprop, modelns, scope, localModel);
	    					if (inclname != null) {
	    	...
	    	case DATATYPEPROPERTY:
	    		ExtendedIterator<DatatypeProperty> dpitr = localModel.listDatatypeProperties();
	    		if (dpitr.hasNext()) {
	    			while (dpitr.hasNext()) {
	    				DatatypeProperty dprop = dpitr.next();
	    				if (!dprop.isAnon()) {
	    					ConceptName inclname = getNamedConceptFromModel(localModel, dprop, modelns, scope, localModel);
	    					if (inclname != null) {
	    		...
	    		break;
	    	case INDIVIDUAL:
	    		ExtendedIterator<Individual> iitr = localModel.listIndividuals();
	    		if (iitr.hasNext()) {
	    			while (iitr.hasNext()) {
	    				Individual inst = iitr.next();
	    				if (!inst.isAnon()) {
	    					ConceptName inclname = getNamedConceptFromModel(localModel, inst, modelns, scope, localModel);
	    					if (inclname != null) {
	    		...
	    		break;
	    	case OBJECTPROPERTY:
	    		ExtendedIterator<ObjectProperty> opitr = localModel.listObjectProperties();
	    		if (opitr.hasNext()) {
	    			while (opitr.hasNext()) {
	    				ObjectProperty oprop = opitr.next();
	    				if (!oprop.isAnon()) {
	    					ConceptName inclname = getNamedConceptFromModel(localModel, oprop, modelns, scope, localModel);
	    					if (inclname != null) {
	    	...
	    	case ONTCLASS:
	    		...
	    		if (citr.hasNext()) {
	    			while (citr.hasNext()) {
	    				...
	    				if (!aclass.isAnon()) {
	    					ConceptName inclname = getNamedConceptFromModel(localModel, aclass, modelns, scope, localModel);
	    					if (inclname != null) {
    ...
    protected ConceptName getNamedConceptFromModel(OntModel om, Resource aprop, String namespace, 
			Scope scope, OntModel localModel) throws InvalidNameException {
    	if (aprop.isURIResource() && ((scope != null && scope.equals(Scope.INCLUDEIMPORTS)) || aprop.getNameSpace().equals(namespace))) {
    ...
    /* (non-Javadoc)
	 * @see com.ge.research.sadl.reasoner.IConfigurationManagerForEditing#getJenaDocumentMgr()
	 */

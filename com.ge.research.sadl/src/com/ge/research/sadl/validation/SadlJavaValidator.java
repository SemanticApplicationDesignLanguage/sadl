	...
	public void checkModelName(ModelName uri) {
		...
		if (baseUri == null || baseUri.isEmpty()) {
	        ...
	        try {
		        ...
		        if (altUri != null && !publicUri.equals(altUri) && !owlUrl.equalsIgnoreCase(altUri)) {
		...
		try {
    		...
    		if (configMgr != null) {
    			if (uri.endsWith(ResourceManager.SADLEXTWITHPREFIX)) {
    				...
    				if (visitor.getModelResource() == null) {
    					if (imp.eResource() != null) {
    						visitor.init(imp.eResource());
    					}
    					else {
    						//TODO can't check until this is fixed
    						return;
    					}
    				}
					...
					String sadlFilepath = ResourceManager.findSadlFileInProject(prjDir, uri, ResourceManager.sadlExclusionFolders(prjDir));
					if (sadlFilepath != null) {
						URI sadlUri = URI.createURI(su.fileNameToFileUrl(sadlFilepath));
						URI owlUri = ResourceManager.validateAndReturnOwlUrlOfSadlUri(sadlUri);
						uri = configMgr.findPublicUriOfOwlFile(owlUri.toString());
					}
					else {
						error("Import URI '" + uri + "' not found in project '" + prjDir + "'.", imp, SadlPackage.Literals.IMPORT__IMPORT_URI, IMPROPER_IMPORT_FILE_URI);
					}
	    		}
...
//				}
	...
	public void checkRead(Read read) {
		String ifn = read.getFilename();
		String tfn = read.getTemplateFilename();
		try {
			File infile = visitor.getModel().getConfigurationMgr().resolveFilename(ifn);
			if (infile == null) {
				error("The specified input file '" + ifn + "' was not found.", read, SadlPackage.Literals.READ__FILENAME, INVALID_READ_FILENAME);	
			}
			if (tfn != null) {
				File tf = visitor.getModel().getConfigurationMgr().resolveFilename(tfn);
				if (tf == null) {
					error("Template file '" + tfn + " not found.", read, SadlPackage.Literals.READ__FILENAME, INVALID_READ_FILENAME);	
				}
				if (infile != null && !infile.getName().toLowerCase().endsWith(".csv")) {
					error("Since a template file is specified, an input file ending in '.csv' was expected.", read, SadlPackage.Literals.READ__TEMPLATE_FILENAME, INVALID_READ_TEMPLATE_FILENAME);	
				}
			}
		} catch (MalformedURLException e) {
			// TODO Auto-generated catch block

package com.ge.research.sadl.cli

import java.io.File
import java.util.Arrays
import com.google.common.base.Optional
import org.eclipse.emf.common.util.URI

class SadlCli {

	public static Optional<URI> projectRootUri = Optional.absent;

	def static void main(String[] args) {
		try {
			val file = getProjectRootPath(newArrayList('/Users/akos.kitta/Downloads/TestSadlIde'));
//			val file = getProjectRootPath(newArrayList('/Users/akos.kitta/dev/wss/sadl_jar/runtime-New_configuration/Test'));
			projectRootUri = Optional.fromNullable(URI.createFileURI(file.toFile.absolutePath));
			new SadlCliHelper().transform(file);
		} finally {
			projectRootUri = Optional.absent;
		}
	}

	private def static getProjectRootPath(String[] args) {
		if (args === null || args.length === 0) {
			throw new IllegalArgumentException('The absolute path to the project root should be specified.');
		}

		if (args.length > 1) {
			throw new IllegalArgumentException('''Only one project can be converted in a time. args was: «Arrays.toString(args)».''');
		}
		
		val file = new File(args.head);
		if (!file.exists) {
			throw new IllegalArgumentException('''Project root «file» does not exist.''');
		}
		
		if (file.file) {
			throw new IllegalArgumentException('''Expected a foldes as the project root. Got a file instead: «file».''');
		}
		
		return file.toPath;
	}

}

...
//					ncr = getOntResourceInExistingModel(new ConceptName(newClassName));
						...
						if (r == null) {
							...
							if (errors() < 1) {
								...
								if (prop.isAnnotationProperty()) {
									...
									try {
										val = getLiteralMatchingDataPropertyRange(
														getJenaModel(), prop,
														inputVal);
									} catch (Exception e) {
										e.printStackTrace();
	...
	private Literal getLiteralMatchingDataPropertyRange(OntModel m, OntProperty prop, Object v) {
        Literal val = null;
        String errMsg = null;
        if (prop.isAnnotationProperty()) {
	...
	private Literal getLiteralMatchingDataPropertyRange(OntModel m, String rnguri, Object v) {
		Literal val = null;
        String errMsg = null;
	...
	public List<ModelError> addDefaultValue(ConceptIdentifier clsname,
		...
		if (ptype.equals(ConceptType.OBJECTPROPERTY)) {
			...
			if (defVal instanceof ConceptName) {
				...
				Literal litVal;
				try {
					litVal = getLiteralMatchingDataPropertyRange(getJenaModel(),
									prop, defVal);
					try {
	...
	private HasValueRestriction createHasValueRestriction(ConceptName prop,
		...
		if (rCond.getRestrictedToConcept() != null) {
			...
			if (!ptype.equals(ConceptType.DATATYPEPROPERTY)) {
				...
				try {
					restrictedTo = getLiteralMatchingDataPropertyRange(
									getJenaModel(), p,
				...
				if (firstElement instanceof ConceptIdentifier) {
					...
					if (!propType.equals(ConceptType.DATATYPEPROPERTY)) {
						...
						for (int i = 0; i < enumList.size(); i++) {
							...
							Literal lval;
							try {
								lval = getLiteralMatchingDataPropertyRange(
...
//			}
	...
	public List<ModelError> addUserDefinedDataType(String name,
			EList<SadlDataType> unionOfTypes, String baseType, String minexin, String min, String maxexin,

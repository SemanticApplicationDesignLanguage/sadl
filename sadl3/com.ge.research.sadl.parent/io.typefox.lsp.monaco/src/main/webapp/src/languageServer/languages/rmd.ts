...
export var language = <ILanguage>{
    ...
    tokenizer: {
        root: [
            ...
            // r code chunk
            [/^\s*```\{r(\s|,)*.*$/, { token: TOKEN_BLOCK, next: '@rChunk', nextEmbedded: 'r'}],

            // github style code blocks (with backticks but no language)
            [/^\s*`{3}\s*$/, { token: TOKEN_BLOCK, next: '@codeblock' }],

            // r inline code
            [/`r\s/, { token: TOKEN_BLOCK, next: '@rInline', nextEmbedded: 'r'}],

            // inline code
            [/`[^r`][^`]*`/, { token: TOKEN_BLOCK_CODE, next: '@inlineCode' }],
        ...
        inlineCode: [
            [/`/, { token: TOKEN_BLOCK, next: '@pop' }],
            [/[^`]*/, TOKEN_BLOCK_CODE],
        ],

        rChunk: [
            [/^\s*`{3}\s*$/, { token: '@rematch', switchTo: '@rChunkEnd', nextEmbedded: '@pop'}],
            [/.*$/, TOKEN_BLOCK_CODE],
        ],

        rChunkEnd: [
            [/\s*```/, { token: TOKEN_BLOCK, next: '@pop' }],
            [/./, '@rematch', '@pop'],
        ],

        rInline: [
            [/`/, { token: '@rematch', switchTo: '@rInlineEnd', nextEmbedded: '@pop'}],
            [/[^`]*/, TOKEN_BLOCK_CODE],
        ],

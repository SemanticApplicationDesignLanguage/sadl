...
\"http://sadl.org/ImpliedPropertiesInRule.sadl#MyRect\",-8.8 \"ft*ft\"", (tr as ResultSet).toString.trim)
	...
	def void testUnittedQuantityInRule_01c() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
		updatePreferences(new PreferenceKey(SadlPreferences.ALWAYS_EXPAND_UNITTEDQUANTITY_IN_TRANSLATION.id, Boolean.FALSE.toString));
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 uri "http://sadl.org/ImpliedPropertiesInRule.sadl" alias impliedpropertiesinrule.
...
\"http://sadl.org/testUQTimesNumber.sadl#MyRect\",25.0 \"ft\"", (tr as ResultSet).toString.trim)
	...
	@Test
	def void testUQInRule_product_uq_and_notuqWithIgnoreUQs() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
...
\"http://sadl.org/UnittedQuantityInRule3.sadl#MyRP\",7.300000000000001 \"ft\"", (tr as ResultSet).toString.trim)
	...
	def void testUQInRule_sum_01() {
		...
		createFile(sfname, '''
			 ...
			 Rule R2: if x is a Duration then p of x is (2 + 3) "seconds".
...
//\"http://sadl.org/UnittedQuantityInRule3.sadl#MyRP\",7.300000000000001 \"ft\"", (tr as ResultSet).toString.trim)
	...
	@Test
	def void testUQInRule_sum_01_no_expansion() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
		updatePreferences(new PreferenceKey(SadlPreferences.ALWAYS_EXPAND_UNITTEDQUANTITY_IN_TRANSLATION.id, Boolean.FALSE.toString));
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 ...
			 Duration is a class, described by p with values of type UnittedQuantity.
			 Duration2 is a class, described by p2 with values of type UnittedQuantity.
			 Duration3 is a class, described by p3 with values of type UnittedQuantity,
			 	described by p4 with values of type UnittedQuantity.
			 
			 Rule R1: if x is a Duration then p of x is 2 seconds + 3 seconds.
			 Rule R2: if x is a Duration2 then p2 of x is (2 + 3) "seconds".
			 Rule r3: if x is a Duration3 and x has p3 y then p4 of x is p3 of x + 1 "seconds".
			 
			 D1 is a Duration.
			 D2 is a Duration2.
			 D3 is a Duration3 with p3 4 "seconds".
			 
			 Ask: select d, v where d has p v.
			 Ask: select d, v where d has p2 v.
			 Ask: select d, v where d has p4 v.
			 ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
			assertNotNull(jenaModel)
			if (issues !== null) {
				for (issue : issues) {
...
«««			 Rule AdultRule2: if p is a Person and p has age uq and unit of uq is "years" and ^value of uq >= 18 years then p is an Adult.
	...
	@Test
	def void testUQInRule_ge_uq_literal_explicit_builtin() {
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 uri "http://sadl.org/test.sadl" alias test.
			 ...
			 Rule AdultRule: if p is a Person and p has age x and ge(x,18 years) then p is an Adult.
«««			 Rule AdultRule2: if p is a Person and p has age uq and unit of uq is "years" and ^value of uq >= 18 years then p is an Adult.
			 Expr: ge(age, 18 years).
			 Test: Jane is an Adult.
 			 ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
...
\"http://sadl.org/test.sadl#Jane\"", (tr as ResultSet).toString.trim)
	...
	@Test
	def void testListOfUnittedQuantitiesInQuery_01() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 uri "http://sadl.org/UnittedQuantityList.sadl" alias unittedquantitylist.
			 
			 Anchor is a class described by p1 with values of type UQList.
			 
			 MyUQList1 is the UnittedQuantity List [3 lbs, 4 lbs, 5 lbs].
			 
			 UQList is a type of UnittedQuantity List.
			 
			 MyUQList2 is the UQList [3 lbs, 4 lbs, 5 lbs].
			 
			 Anchor1 is an Anchor, has p1 MyUQList1, has p1 MyUQList2, has p1 [6 lbs, 7 lbs, 8 lbs].
			  			 
			 Ask: select x, y where x is an Anchor and x has p1 y order by x, y.
 			 ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
			assertNotNull(jenaModel)
//			jenaModel.write(System.out)
			if (issues !== null) {
		...
		assertInferencer(sfname, null, configItems) [
			for (scr : it) {
				...
				assertTrue(tr instanceof ResultSet)
				assertEquals("\"x\",\"y\"
\"http://sadl.org/UnittedQuantityList.sadl#Anchor1\",[6 \"lbs\",7 \"lbs\",8 \"lbs\"]
\"http://sadl.org/UnittedQuantityList.sadl#Anchor1\",\"http://sadl.org/UnittedQuantityList.sadl#MyUQList1\"
\"http://sadl.org/UnittedQuantityList.sadl#Anchor1\",\"http://sadl.org/UnittedQuantityList.sadl#MyUQList2\"", (tr as ResultSet).toString.trim)
				(tr as ResultSet).showNamespaces = false;
				assertEquals("\"x\",\"y\"
\"Anchor1\",[6 \"lbs\",7 \"lbs\",8 \"lbs\"]
\"Anchor1\",\"MyUQList1\"
\"Anchor1\",\"MyUQList2\"", (tr as ResultSet).toString.trim)
			}
		];
	}
	...
	def void testUnittedQuantityInTest_06() {
		...
		createFile(sfname, '''
 	    ...
 	    ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
			...
			assertEquals("test3:weight, a datatype property with range  xsd:float, cannot be compared (>) with sadlimplicitmodel:UnittedQuantity, a class .", 
	...
	def void testExpandUnittedQuantityPreference_01() {
		...
		''').resource.assertValidatesTo [ jenaModel, rules, commands, issues, processor |
			...
			assertEquals("Rule R1:  if rdf(x, rdf:type, uqir:Rectangle) and rdf(x, uqir:height, v0) and rdf(x, uqir:width, v1) and rdf(v0, sadlimplicitmodel:value, v0_val) and rdf(v0, sadlimplicitmodel:unit, v0_unit) and rdf(v1, sadlimplicitmodel:value, v1_val) and rdf(v1, sadlimplicitmodel:unit, v1_unit) and combineUnits(\"Multiply\",v0_unit,v1_unit,v2_unit) and *(v0_val,v1_val,v2) then thereExists(UnittedQuantity,sadlimplicitmodel:value,v2,sadlimplicitmodel:unit,v2_unit,sadlimplicitmodel:Plus,x,uqir:area).", 
				rules.get(0).toString
			)
		]
...
«««			  Test: unit of area of RectX is "ft*ft".
	...
	def void testUQInRule_Abs() {
		...
		createFile(sfname, '''
			...
			assertEquals("Rule R1:  if rdf(x, rdf:type, absuq:NumberAndAbsValue) and rdf(x, absuq:number, v0) and rdf(v0, sadlimplicitmodel:value, v0_val) and rdf(v0, sadlimplicitmodel:unit, v0_unit) and abs(v0_val,v1) then thereExists(UnittedQuantity,sadlimplicitmodel:value,v1,sadlimplicitmodel:unit,v0_unit,sadlimplicitmodel:Plus,x,absuq:absValue).",
				rules.get(0).toString
			)
		]
	...
	def void testUQInRule_AbsEmbedded() {
		...
		createFile(sfname, '''
			...
			assertTrue(errors.size == 0)
			assertTrue(rules.size == 1)
			assertEquals("Rule R1:  if rdf(x, rdf:type, absuqembedded:NumberAndAbsValue) and rdf(x, absuqembedded:number1, v0) and rdf(x, absuqembedded:number2, v1) and rdf(v0, sadlimplicitmodel:value, v0_val) and rdf(v0, sadlimplicitmodel:unit, v0_unit) and rdf(v1, sadlimplicitmodel:value, v1_val) and rdf(v1, sadlimplicitmodel:unit, v0_unit) and +(v0_val,v1_val,v2) and abs(v2,v3) then thereExists(UnittedQuantity,sadlimplicitmodel:value,v3,sadlimplicitmodel:unit,v0_unit,sadlimplicitmodel:Plus,x,absuqembedded:absValue).", rules.get(0).toString)
		]
	...
	@Test
	def void testCompoundUnittedQuantityErroneous() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 uri "http://sadl.org/CompoundUnittedQuantity.sadl" alias compoundunittedquantity.
			 ...
			 Rule R1: if x is a Rectangle and unit of height of x is "ft" and unit of width of x is "ft" 
			 	then area of x is (height of x * width of x) "sq ft".
			 Ask: select r, ar where r has area ar.	
 			 ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
			assertNotNull(jenaModel)
			if (issues !== null) {
				for (issue : issues) {
			...
			assertTrue(errors.size == 0)
			assertTrue(rules.size == 1)
			assertEquals("The value part of the unitted quantity expression is already a unitted quantity", issues.get(0).message)
			assertEquals("Rule R1:  if rdf(x, rdf:type, compoundunittedquantity:Rectangle) and rdf(x, compoundunittedquantity:height, v0) and rdf(v0, sadlimplicitmodel:unit, \"ft\") and rdf(x, compoundunittedquantity:width, v1) and rdf(v1, sadlimplicitmodel:unit, \"ft\") and rdf(v0, sadlimplicitmodel:value, v0_val) and rdf(v0, sadlimplicitmodel:unit, v0_unit) and rdf(v1, sadlimplicitmodel:value, v1_val) and rdf(v1, sadlimplicitmodel:unit, v1_unit) and combineUnits(\"Multiply\",v0_unit,v1_unit,v2_unit) and *(v0_val,v1_val,v2) then thereExists(UnittedQuantity,sadlimplicitmodel:value,v2,sadlimplicitmodel:unit,\"sq ft\",sadlimplicitmodel:Plus,x,compoundunittedquantity:area).", 
				rules.get(0).toString
			)
		]
		...
		assertInferencer(sfname, null, configItems) [
			for (scr : it) {
				...
				assertTrue(tr instanceof ResultSet)
				var resultUQ = (tr as ResultSet).getResultAt(0,1)
				assertEquals("8.8 \"sq ft\"", resultUQ.toString)
			}
		];
	}
	...
	@Test
	def void testCompoundUnittedQuantityFixed() {
		updatePreferences(new PreferenceKey(SadlPreferences.TYPE_CHECKING_WARNING_ONLY.id, Boolean.TRUE.toString));
		val sfname = 'JavaExternal.sadl'
		createFile(sfname, '''
			 uri "http://sadl.org/CompoundUnittedQuantity.sadl" alias compoundunittedquantity.
			 ...
			 Rule R1: if x is a Rectangle and unit of height of x is "ft" and unit of width of x is "ft" 
			 	then area of x is (^value of height of x * ^value of width of x) "sq ft".
			 Ask: select r, ar where r has area ar.	
 			 ''').resource.assertValidatesTo [ jenaModel, rules, cmds, issues, processor |
			assertNotNull(jenaModel)
			if (issues !== null) {
			...
			assertTrue(errors.size == 0)
			assertTrue(rules.size == 1)
			assertEquals("Rule R1:  if rdf(x, rdf:type, compoundunittedquantity:Rectangle) and rdf(x, compoundunittedquantity:height, v0) and rdf(v0, sadlimplicitmodel:unit, \"ft\") and rdf(x, compoundunittedquantity:width, v1) and rdf(v1, sadlimplicitmodel:unit, \"ft\") and rdf(v0, sadlimplicitmodel:value, v2) and rdf(v1, sadlimplicitmodel:value, v3) and *(v2,v3,v4) then thereExists(UnittedQuantity,sadlimplicitmodel:value,v4,sadlimplicitmodel:unit,\"sq ft\",sadlimplicitmodel:Plus,x,compoundunittedquantity:area).", rules.get(0).toString)
		]

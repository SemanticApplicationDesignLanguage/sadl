...
class SadlExpressionParserTest extends AbstractSADLParsingTest {
	...
	@Test def void testUnits_01() {
		'''
			42 seconds
		'''.<SubjHasProp>expression => [
			assertEquals(42, (left as NumberLiteral).value.toBigInteger.intValue)
		]
	}
	...
	@Test def void testUnits_02() {
		...
		'''.<BinaryOperation>expression => [ op |
			val it = op.right as SubjHasProp
			assertEquals(42, (left as NumberLiteral).value.toBigInteger.intValue)
		]
	}
	...
	@Test def void testUnits_03() {
		'''
			(3 + 42) seconds
		'''.<SubjHasProp>expression => [
			assertEquals(42, ((left as BinaryOperation).right as NumberLiteral).value.toBigInteger.intValue)
		]
	}
}
